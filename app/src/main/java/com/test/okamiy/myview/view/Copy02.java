...
public class Copy02 extends View {
    ...
    public Copy02(Context context) {
        super(context);
        init();
    }

    public Copy02(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public Copy02(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void init() {
        radius = 300;
        datas = new ArrayList<>();
        Data data = new Data("Froyo", 10.0f, Color.WHITE);
        datas.add(data);
        data = new Data("Gingerbread", 18.0f, Color.GREEN);
        datas.add(data);
        data = new Data("Ice Cream Sandwich", 22.0f, Color.GRAY);
        datas.add(data);
        data = new Data("OKamiy", 27.0f, Color.MAGENTA);
        datas.add(data);
        data = new Data("KitKat", 40.0f, Color.BLUE);
        datas.add(data);
        data = new Data("Lollipop", 65.0f, Color.RED);
        datas.add(data);
        data = new Data("Marshmallow", 35.5f, Color.YELLOW);
        datas.add(data);
        total = 0.0f;
        max = Float.MIN_VALUE;
